...
export default class extends Controller {
  ...
  async _copyTextToClipboard() {
    const textToCopy =
      ...
      this.element.getAttribute("data-clipboard-text");
    if (textToCopy === null || typeof textToCopy === "undefined") {
      console.warn(
        "No text to copy. Missing data-clipboard-text attribute on element:",
        this.element,
      );
  ...
  _createOrUpdateTooltipDOMSafer(message) {
    if (!this.tooltipElement) {
      ...
      this.arrowElement.className =
        "tooltip-arrow-element bg-[#333333] w-2 h-2 border-white/10";
    ...
    const messageSpan = this.tooltipElement.querySelector(
      "[data-tooltip-message-span]",
    );
    if (messageSpan) {
  ...
  async _showFloatingTooltip(message) {
    ...
    this.cleanupAutoUpdate = autoUpdate(
      ...
      async () => {
        ...
        const placements = this.tooltipPlacementValue
          .split(/[\s,]+/)
          .filter(Boolean);
        ...
        const middleware = [
          ...
          flip({
            fallbackPlacements:
              fallbackPlacements.length > 0 ? fallbackPlacements : undefined,
          }),
          shift({ padding: 5 }),
        ];
